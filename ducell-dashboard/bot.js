...
function runClaude(prompt, chatId) {
  return new Promise((resolve, reject) => {
    ...
    const env = Object.assign({}, process.env, { TELEGRAM_CHAT_ID: String(chatId) });
...
${memory}
${conversationSection}
TELEGRAM UPDATES — CRITICAL:
You have a tool to send real-time messages to the user while you work. Use it like this:
  bash /root/ducell/send_update.sh "$TELEGRAM_CHAT_ID" "your message here"

You MUST call this before and after every meaningful action. The user sees nothing until you send an update — so send one at EVERY step. Examples of when to send:
- Before reading a file: "Reading the current bot.js to understand the structure."
- Before writing code: "Writing the HTML file now."
- Before deploying: "Deploying to Vercel — this usually takes 30-60 seconds."
- After deploy: "Deployed. Testing the live URL now."
- If something fails: "Deploy failed — fixing the issue and retrying (attempt 2/3)."
- Before pushing to GitHub: "Pushing project files to GitHub."

Never go more than one action without sending an update. The final text you output at the end is also sent to the user, so do NOT duplicate the last step in both a send_update call and your final output.
...
6. STATUS UPDATES — Use send_update.sh at every meaningful step. Never go silent between tool calls.
